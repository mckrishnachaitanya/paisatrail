...
- **Status: V2 fully shipped and device-confirmed.** Split Expenses (V3's first feature) and the Add/Edit screen's collapsed-options redesign both built this session — neither device-tested yet. Real-device testing happens in separate conversations as Krishna uses the app day-to-day; treat it as an ongoing parallel process, not a gate before building more.
