...
3. **Actions pill (idea, NEXT feature)** — a small tappable badge/pill near the header (e.g. "1 Action") opening a consolidated action-center that pulls together the currently-scattered nudges: warranty alerts, backup nudge, and (now that it exists) potentially a "your balance may need reconciling" prompt. Reconciliation was cited as the first real "action" this would surface — build the consolidation layer around real content that now exists. Explicitly EXCLUDE initial setup/onboarding from the pill (that's a one-time linear flow, not a recurring action — Krishna confirmed). Build only after reconciliation has been used/confirmed stable on device.
